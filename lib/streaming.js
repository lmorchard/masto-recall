...
export default class StreamingPlugin extends BasePlugin {
  ...
  async handleStatusUpdate(payloadRaw, event) {
    ...
    if (this.seenItemId(id)) return;

    const { acct } = account;
    if (visibility !== "public") return;

    log.info({ msg: "statusUpdate", event, id });
  ...
  /**
   * Track recently seen IDs for stream events.
   * 
   * There seems to be weird streaming API behavior where, over time,
   * duplicate messages start getting received in great volume.
   * 
   * This is probably indicative of another issue, e.g. multiple
  ...
  seenItemId(id) {
    ...
    if (this.seenItemIds.has(id)) return true;
    this.seenItemIds.set(id, Date.now());

    // Expire seen ids past max age
    const maxAge = config.get("streamingDuplicateMaxAge");
    [...this.seenItemIds.entries()]
      .filter(([_, time]) => (Date.now() - time > maxAge))
      .map(([id, _]) => id)
      .forEach(id => this.seenItemIds.delete(id));
